...
# Load in data
geo_data = gpd.read_file('tl_2014_us_county.shp')
...
indexNames = geo_data[(geo_data['STATEFP'] == 15) | (geo_data['STATEFP'] == 2)].index
 
# Delete these row indexes from dataFrame
geo_data.drop(indexNames , inplace=True)
...
#geo_data['COUNTYFP'] = geo_data['FIPS'].astype(str)
food_access['FIPS'] = food_access['FIPS'].astype(str)
merged = geo_data.merge(food_access, left_on='GEOID',
                        right_on='FIPS', how='inner')



merged.plot(column='PCT_LACCESS_POP10', figsize=(15, 7), legend=True)
